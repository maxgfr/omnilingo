...
describe("bridge", () => {
  ...
  // ── Favorites ───────────────────────────────────────────────────────
  describe("favorites", () => {
    it("toggleFavorite passes wordId", async () => {
      mockInvoke.mockResolvedValueOnce(true);
      const bridge = await getBridge();
      const result = await bridge.toggleFavorite(5);
      expect(mockInvoke).toHaveBeenCalledWith("toggle_favorite", { wordId: 5 });
      expect(result).toBe(true);
    });

    it("getFavorites validates with Zod schema", async () => {
      mockInvoke.mockResolvedValueOnce([
        { id: 1, word_id: 10, source_word: "Haus", target_word: "maison", gender: "n", level: "A1", category: null, tags: null, example_source: null, example_target: null },
      ]);
      const bridge = await getBridge();
      const favs = await bridge.getFavorites(1);
      expect(favs).toHaveLength(1);
      expect(favs[0].source_word).toBe("Haus");
    });

    it("getFavorites rejects malformed data", async () => {
      mockInvoke.mockResolvedValueOnce([{ bad: "data" }]);
      const bridge = await getBridge();
      await expect(bridge.getFavorites(1)).rejects.toThrow();
    });

    it("isFavorite passes wordId", async () => {
      mockInvoke.mockResolvedValueOnce(false);
      const bridge = await getBridge();
      const result = await bridge.isFavorite(10);
      expect(mockInvoke).toHaveBeenCalledWith("is_favorite", { wordId: 10 });
      expect(result).toBe(false);
    });

    it("createFavoriteList passes name and pairId", async () => {
      mockInvoke.mockResolvedValueOnce({ id: 1, name: "Test", language_pair_id: 1, item_count: 0, created_at: "2025-01-01" });
      const bridge = await getBridge();
      const list = await bridge.createFavoriteList("Test", 1);
      expect(mockInvoke).toHaveBeenCalledWith("create_favorite_list", { name: "Test", pairId: 1 });
      expect(list.name).toBe("Test");
    });

    it("deleteFavoriteList passes listId", async () => {
      mockInvoke.mockResolvedValueOnce(undefined);
      const bridge = await getBridge();
      await bridge.deleteFavoriteList(5);
      expect(mockInvoke).toHaveBeenCalledWith("delete_favorite_list", { listId: 5 });
    });

    it("renameFavoriteList passes listId and name", async () => {
      mockInvoke.mockResolvedValueOnce(undefined);
      const bridge = await getBridge();
      await bridge.renameFavoriteList(5, "New Name");
      expect(mockInvoke).toHaveBeenCalledWith("rename_favorite_list", { listId: 5, name: "New Name" });
    });

    it("getFavoriteLists passes pairId", async () => {
      mockInvoke.mockResolvedValueOnce([]);
      const bridge = await getBridge();
      await bridge.getFavoriteLists(1);
      expect(mockInvoke).toHaveBeenCalledWith("get_favorite_lists", { pairId: 1 });
    });

    it("addToFavoriteList passes listId and wordId", async () => {
      mockInvoke.mockResolvedValueOnce(undefined);
      const bridge = await getBridge();
      await bridge.addToFavoriteList(1, 10);
      expect(mockInvoke).toHaveBeenCalledWith("add_to_favorite_list", { listId: 1, wordId: 10 });
    });

    it("removeFromFavoriteList passes listId and wordId", async () => {
      mockInvoke.mockResolvedValueOnce(undefined);
      const bridge = await getBridge();
      await bridge.removeFromFavoriteList(1, 10);
      expect(mockInvoke).toHaveBeenCalledWith("remove_from_favorite_list", { listId: 1, wordId: 10 });
    });

    it("getFavoriteListItems passes listId", async () => {
      mockInvoke.mockResolvedValueOnce([]);
      const bridge = await getBridge();
      await bridge.getFavoriteListItems(1);
      expect(mockInvoke).toHaveBeenCalledWith("get_favorite_list_items", { listId: 1 });
    });

    it("getWordListMemberships returns number array", async () => {
      mockInvoke.mockResolvedValueOnce([1, 3, 5]);
      const bridge = await getBridge();
      const ids = await bridge.getWordListMemberships(10, 1);
